...
wentlst = [set() for _ in range(n + 1)]
q = [[1, 0, set()]]

while q:
    dot, cost, ns = heapq.heappop(q)
    if cost in wentlst[dot]:
        continue
    wentlst[dot].add(cost)
    news = ns.copy()
    news.add(dot)
    ...
    for i in line[dot]:
        if i[0] in ns:
            continue
        newcost = cost | i[1]
        if newcost not in wentlst[i[0]]:
            heapq.heappush(q, [i[0], newcost, news])
print(min(wentlst[-1]))
