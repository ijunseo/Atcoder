...
went = [set() for _ in range(n + 1)]

start = 1
q = [[1, 0]]
while q:
    nowdot, nowcost = q.pop()
    if nowcost in went[nowdot]:
        continue
    
    went[nowdot].add(nowcost)

    for lines in line[nowdot]:
        nextdot, cost = lines[0], lines[1]
        q.append([nextdot, nowcost ^ cost])
