...
lllst = list(map(int, input().split()))
ans = -math.inf
...
while q:
    #print(q)
    nowr, nowc, cost, cst = q.popleft()
    if nowr == n - 1 and nowc == m - 1:
        ans = max(ans, cst)
    cost += lst[nowr][nowc] - lllst[nowr + nowc]

    if cost < 0:
        cst = min(cst, cost)

    if llst[nowr][nowc] <= cost:
        continue
    ...
    for i in range(2):
        ...
        if 0 <= nextr < n and 0 <= nextc < m:
            if llst[nextr][nextc] > cost + lst[nextr][nextc]:
                q.append((nextr, nextc, cost, cst))
