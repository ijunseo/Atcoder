...
lllst = [0]
for i in llst:
    lllst.append(lllst[-1] + i[1])
for i in llst:
    lllst.append(lllst[-1] + i[1])

#print(tar, mi)
#print(lllst)
for i in range(1, len(llst) + 1):
    now = lllst[i]
    if i == len(llst):
        lengt = m + llst[0][0] - llst[i - 1][0]
    else:
        lengt = llst[i][0] - llst[i - 1][0]

    target = tar + now
    l = bisect.bisect_left(lllst, target)
    #print(now, '//')
    #print(target, '!')
    #print(lllst[l], '!!!')
    ans += lengt * (lllst[l]- now + mi)
print(ans)
