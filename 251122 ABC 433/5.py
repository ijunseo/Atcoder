...
for i in range(9):
    ...
    for ll in lft:
        num = bisect.bisect_right(rgt, ll)
        r_num = len(rgt) - num
        
        if r_num > 0:
            l_num = bisect.bisect_right(lft, ll)

            print(i, num, r_num, l_num)
            
            for k in range(1, min(l_num, r_num) + 1):
                print(k, r_num, '!!!')
                now_ans = (math.comb(max(l_num, r_num), k)) % MOD
                print(now_ans)
                ans = (ans + now_ans) % MOD
